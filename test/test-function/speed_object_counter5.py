# speed_based_counter_improved_ema.py
...
class SpeedBasedCounter(SpeedEstimator):
    ...
    def __init__(self, speed_threshold=0, t=0, scale_factor=1.0, average_frames=5, alpha=0.3, max_history=10, **kwargs):
        """
        ...
        self.object_frame_counters = {}  # {track_id: {'frames': int, 'counted': bool, 'speed_accumulator': float, 'speed_samples': int, 'ema_speed': float}}
    ...
    def estimate_speed_and_count(self, im0, fps):
        ...
        for track_id, box, cls in zip(self.track_ids, self.boxes, self.clss):
            ...
            # Update tracking history with raw centroid
            ...
            # Initialize EMA speed if not present
            if track_id not in self.object_frame_counters:
                self.object_frame_counters[track_id] = {
                    ...
                    'speed_samples': 0,
                    'ema_speed': speed  # Initialize EMA with first speed
                }
            else:
                # Update EMA speed
                previous_ema = self.object_frame_counters[track_id]['ema_speed']
                ema_speed = self.alpha * speed + (1 - self.alpha) * previous_ema
                self.object_frame_counters[track_id]['ema_speed'] = ema_speed
                speed = ema_speed  # Use EMA speed for further processing
