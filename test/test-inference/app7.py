...
logging.getLogger("ultralytics.engine.predictor").setLevel(logging.ERROR)
logging.getLogger("ultralytics.trackers").setLevel(logging.ERROR)
...
# Thay thế class ViolationDisplay bằng class mới
class ViolationTracker:
    def __init__(self):
        self.violations = {
            'large_vehicles': 0,  # Region 1 (class 1,2,3)
            'motorcycles': 0,     # Region 2 (class 0)
            'illegal_parking': 0, # Region 3 (v=0)
            'red_light': 0        # Outside Region 3
        }
        self.last_email_time = 0
        self.email_cooldown = 20  # Cooldown in seconds

        # Email settings (using provided credentials)
        self.from_email = "vhp08072004@gmail.com"
        self.password = "ypir iugo vmjl wxoc"
        self.to_email = "vhp08071974@gmail.com"
    ...
    def print_violations(self):
        ...
        print(f"Motorcycles in Vehicle Lane: {self.violations['motorcycles']}")
        print(f"Illegal Parking: {self.violations['illegal_parking']}")
        print(f"Red Light Running: {self.violations['red_light']}")
        print("-" * 50)
    ...
    def send_violation_email(self, violation_type, count):
        try:
            ...
            pass  # Suppress errors to prevent terminal output
        ...
        while self._run_flag:
            if not self.paused:
                ...
                if ret:
                    ...
                    # Process frame with counter_region3
                    detections_region3 = self.counter_region3.count(frame)
                    red_light_violation_count = len(detections_region3)

                    # Process speed estimation
                    illegal_parking_count = self.violation_tracker.violations.get('illegal_parking', 0)
                    ...
                    self.violation_tracker.update_violations('red_light', red_light_violation_count)
...
class MainWindow(QMainWindow):
    ...
    def display_video(self, filepath):
        ...
        self.video_thread = VideoThread(filepath, self.speed_estimator, self.counter_region1, self.counter_region2, self.counter_region3, self.violation_tracker)
    ...
    def handle_cars_counted(self, count, count_type):
        # Update violation display instead of showing popup
        if count_type == "Region 1":
            self.violation_tracker.update_violations('large_vehicles', count)
        elif count_type == "Region 2":
            self.violation_tracker.update_violations('motorcycles', count)
        elif count_type == "Illegal Parking":
            self.violation_tracker.update_violations('illegal_parking', count)
        elif count_type == "Red Light":
